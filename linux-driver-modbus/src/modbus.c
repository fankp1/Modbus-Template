...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length)
{
    ...
    do {
         if(ctx->rw_pin!=0)
	 {
	 	GPIOWrite(ctx->rw_pin,HIGH);
	 	rc = ctx->backend->send(ctx, msg, msg_length);
         	usleep(msg_length*ctx->t_char*10);
		GPIOWrite(ctx->rw_pin,LOW);
	 }
	 else
	 {
	 	rc = ctx->backend->send(ctx, msg, msg_length);
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    /* Suppress any Hesponses when the request was a broadcast */
    if(ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU && slave == MODBUS_BROADCAST_ADDRESS) 
    { 
	    return 0; 
    }
    else
	...
	int ret = send_msg(ctx, rsp, rsp_length);
